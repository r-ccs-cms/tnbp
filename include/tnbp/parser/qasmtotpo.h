...
namespace tnbp {
  ...
  std::vector<ElemT> InstructionMatrix(const qasm::Instruction & ins) {
    ...
    case qasm::Op::CX: {
      ...
      m[0*4+0]=1; m[2*4+2]=1; m[1*4+3]=1; m[3*4+1]=1;
      return m;
    }
    case qasm::Op::CZ: {
      // |11>に -1 の位相
    ...
    case qasm::Op::CCX: {
      ...
      for(int i=0;i<8;i++) m[i*8+i]=1; // |000>..|101> 固定
      m[3*8+3]=0;
      m[3*8+7]=1; // |110>->|111>
      m[7*8+3]=1; // |111>->|110>
      m[7*8+7]=0;
    ...
    case qasm::Op::CSWAP: {
      ...
      std::vector<ElemT> m(64,ElemT(0,0));
      // |000>..|100>,|111> はそのまま
      m[0*8+0]=1; m[1*8+1]=1; m[2*8+2]=1; m[5*8+5]=1;
      m[6*8+6]=1; m[7*8+7]=1;
      // swap |101> <-> |110> ( 5 <-> 3 )
      m[3*8+5]=1;
      m[5*8+3]=1;
  ...
  /**
     Function to initialize the TPO
  ...
  /**
     Function to absorb gate to the current TPO
     @param[in] ctx: context_handle for tensor operations
